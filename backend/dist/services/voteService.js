...
class VoteService {
     ...
     */
    static async processAndSaveVote(sessionId, voteId, voteType, aiResult, imageUrl) {
     ...
     * Trích xuất ứng cử viên từ kết quả AI
     */
    static extractSelectedCandidate(aiResult) {
        if (!aiResult)
            return null;
     ...
     * Tính độ tin cậy
     */
    static calculateConfidence(aiResult) {
        if (!aiResult)
            return 0;
     ...
     */
    static async validateVote(voteId, isValid, notes) {
        const vote = await voteRepository.findOneBy({ id: voteId });
        if (!vote)
            return null;
        vote.status = isValid ? "valid" : "invalid";
        if (notes) {
     ...
     */
    static async getSessionVotes(sessionId, filters, limit = 20, offset = 0) {
     ...
     */
    static countVotesByCandidate(votes) {
    ...
    static async submitVote(voteData) {
        ...
        const vote = voteRepository.create(votePayload);
        return (await voteRepository.save(vote));
    }
    ...
    static async getVoteResults() {
        ...
        const results = Array.from(voteCounts.entries()).map(([candidate, count]) => ({
            candidate,
            votes: count,
    ...
    static async getVoteStats() {
        ...
        return {
            ...
            lastVoteTime: (await voteRepository.findOne({
