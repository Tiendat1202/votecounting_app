...
import { addSessionMember, closeSessionEarly, getActiveUsers, getSessionDetail, getSessionMembers, removeSessionMember, type ActiveUser, type SessionMember, type SessionRecord } from "../../api";
...
const SessionDetail: React.FC = () => {
  ...
  const [memberRole, setMemberRole] = useState<"inspector" | "supervisor">("inspector");
  ...
  const loadDetail = async () => {
    try {
      ...
      setSession(data);
      if (data?.permissions?.isOwner || user?.role === "admin") {
        const [memberRows, userRows] = await Promise.all([
          getSessionMembers(id).catch(() => []),
          getActiveUsers().catch(() => []),
        ]);
        setMembers(Array.isArray(memberRows) ? memberRows : []);
        setActiveUsers(Array.isArray(userRows) ? userRows : []);
      }
    } catch (e: any) {
  ...
  const handleAddMember = async () => {
    ...
    try {
      setBusy(true);
      await addSessionMember(session.id, memberUserId, memberRole);
      const memberRows = await getSessionMembers(session.id);
      setMembers(memberRows);
      setMemberUserId("");
      alert("Đã thêm/cập nhật thành viên phiên");
    } catch (e: any) {
      alert(e?.message || "Không thêm được thành viên phiên");
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleRemoveMember = async (member: SessionMember) => {
    if (!session) return;
    if (!window.confirm(`Xóa ${member.userFullName || member.userEmail} khỏi phiên này?`)) return;
    try {
      setBusy(true);
      await removeSessionMember(session.id, member.id);
      setMembers((rows) => rows.filter((row) => row.id !== member.id));
    } catch (e: any) {
      alert(e?.message || "Không xóa được thành viên phiên");
    } finally {
      setBusy(false);
    }
  };
      ...
      <div className="create-session-shell">
        <section className="card-surface form-section">
          ...
          <div className="detail-content">
            ...
            {canManage && (
              <div className="detail-candidates">
                <h2 className="detail-candidates-title">Thành viên phiên</h2>
                <div style={{ display: "grid", gap: 10 }}>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <select className="form-control" value={memberUserId} onChange={(e) => setMemberUserId(e.target.value)} style={{ minWidth: 260 }}>
                      <option value="">Chọn người dùng đã được duyệt</option>
                      {activeUsers
                        .filter((candidate) => !members.some((member) => member.userId === candidate.userId && member.role === "owner"))
                        .map((candidate) => (
                    ...
                    <select className="form-control" value={memberRole} onChange={(e) => setMemberRole(e.target.value as "inspector" | "supervisor")}>
                      <option value="inspector">Inspector</option>
                      <option value="supervisor">Supervisor</option>
                    </select>
                    <button type="button" className="btn btn--primary" onClick={handleAddMember} disabled={busy || !memberUserId}>
                      Thêm / cập nhật
                    </button>
                  </div>
                  ...
                  <div style={{ display: "grid", gap: 8 }}>
                    {members.length === 0 ? (
                      <div className="detail-card-note">Chưa có thành viên nào.</div>
                    ) : members.map((member) => (
                      <div key={member.id} style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", padding: "10px 12px", border: "1px solid var(--border)", borderRadius: 12 }}>
                        <div>
                          <strong>{member.userFullName || member.userEmail}</strong>
                          <div className="detail-card-note">{member.userEmail} · {member.role}</div>
                        </div>
                        {member.role !== "owner" && (
                          <button type="button" className="btn btn--subtle" onClick={() => handleRemoveMember(member)} disabled={busy}>
                            Xóa
                          </button>
                        )}
                      </div>
