...
def process_ballot_wrapper(file_bytes: bytes, filename: str, ballot_type: str, idx: int, official_candidates: list = None) -> dict:
    ...
    try:
        ...
        try:
            signature = inspect.signature(process_ballot)
            if official_candidates and len(signature.parameters) >= 5:
            ...
            # Fallback for wrappers / dynamically loaded callables that don't expose a clean signature.
            try:
                result = process_ballot(tmp_path, ballot_type, f"batch_{idx}", filename, official_candidates)
            except TypeError:
                result = process_ballot(tmp_path, ballot_type, f"batch_{idx}", filename)
